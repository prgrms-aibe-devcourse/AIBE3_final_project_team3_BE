...
-- ----------------------------------------------------
-- 💡 초기화 데이터 (종류별 1개, Sender: 2, 3, 4, 5, 6, NULL)
-- ----------------------------------------------------
-- 1. CHAT_MESSAGE (from 2)
(1, 2, 'CHAT_MESSAGE', '초기 알림: 믹스챗 서비스를 시작해 보세요! Welcome to MixChat.', FALSE, NOW() - INTERVAL '40 minute'),
-- 2. FRIEND_REQUEST (from 3)
(1, 3, 'FRIEND_REQUEST', NULL, FALSE, NOW() - INTERVAL '39 minute'),
-- 3. FRIEND_REQUEST_ACCEPT (from 4)
(1, 4, 'FRIEND_REQUEST_ACCEPT', NULL, TRUE, NOW() - INTERVAL '38 minute'),
-- 4. FRIEND_REQUEST_REJECT (from 5)
(1, 5, 'FRIEND_REQUEST_REJECT', NULL, TRUE, NOW() - INTERVAL '37 minute'),
-- 5. CHAT_INVITATION (from 6)
(1, 6, 'CHAT_INVITATION', NULL, FALSE, NOW() - INTERVAL '36 minute'),
-- 6. SYSTEM_ALERT (from NULL)
(1, NULL, 'SYSTEM_ALERT', '시스템 초기 알림: 개인정보 보호 설정 확인. Check your privacy settings.', FALSE, NOW() - INTERVAL '35 minute'),

-- ----------------------------------------------------
-- 💡 나머지 알림 34개 (다양성 확보 및 시간순 정렬)
-- ----------------------------------------------------

-- CHAT_MESSAGE (14개 - Sender 7부터 20까지)
...
-- FRIEND REQUEST / ACCEPT / REJECT (11개 - Sender 21부터 31까지)
...
-- SYSTEM_ALERT (5개, sender_id = NULL)
...
-- CHAT_INVITATION (4개, Sender 32부터 35까지)
